...
| `gapminder` | `global_tech_adoption` | `population`→`active_user_base`, `fertility`→`devices_owned_per_capita`, `infant_mortality`→`app_uninstall_rate_per_1000` (all unchanged); `life_expectancy`→`avg_daily_screen_time_hours` (rescaled ~30–85 → ~1–10); `country`/`year`/`continent`/`region`/`gdp` unchanged | The same longitudinal framework used for infant mortality/GDP across countries applies to the digital divide: internet penetration, smartphone adoption, digital-literacy scores across countries/income levels. Downstream convention: `gdp/active_user_base/365` is used as `revenue_per_user_day`, replacing the "dollars a day" poverty framing with a digital-monetization-divide story; the `opec` country vector is reused as-is, relabeled "Emerging Tech Market" in legends/text. |
